...
const Home: NextPage = () => {
  ...
  const getActiveBlockchainDisplay = () => {
    if (ethereumEnabled && tronEnabled) {
      return activeBlockchain === "ethereum" ? "Ethereum" : "Tron";
  ...
  const shouldShowConnectionStatus = ethereumEnabled || tronEnabled;
      ...
      <div className="flex items-center flex-col flex-grow pt-10">
        <div className="px-5">
          <h1 className="text-center">
            ...
            <span className="block text-sm text-gray-600 mt-2">TRON Scaffolding & Prototyping</span>
          </h1>
          {shouldShowConnectionStatus && (
            <div className="flex justify-center items-center space-x-2 flex-col sm:flex-row">
              <p className="my-2 font-medium">Connected to:</p>
              <p className="my-2 font-bold text-lg">
                {getActiveBlockchainDisplay()}
                {activeBlockchain === "tron" && tronNetwork?.name && ` (${tronNetwork.name})`}
              </p>
            </div>
          )}
          {ethereumEnabled && tronEnabled && (
        ...
        {shouldShowConnectionStatus && (
          <div className="bg-base-200 w-full mt-8 px-8 py-6">
            <div className="max-w-4xl mx-auto">
              ...
              <div className={`grid grid-cols-1 ${ethereumEnabled && tronEnabled ? "md:grid-cols-2" : ""} gap-6`}>
                {/* Ethereum Status */}
                {ethereumEnabled && (
